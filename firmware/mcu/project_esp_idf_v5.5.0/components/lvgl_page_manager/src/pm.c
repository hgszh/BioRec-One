...
uint8_t lv_pm_back() {
    ...
    uint8_t pid = lv_pm_history[lv_pm_history_len - 1];
    lv_pm_page_t *pm_page = lv_pm_router[pid]; // 将要消失的当前页面
    lv_obj_t *page = pm_page->page;
    // 1. 定义动画选项：返回时，上一页总是从左边滑入
    lv_pm_open_options_t back_options = {.animation = LV_PM_ANIMA_SLIDE_LEFT};
    // 2. 让当前页面执行消失动画 (使用 back_options)
    if (pm_page->willDisappear) {
        pm_page->willDisappear(page);
    }
    _pm_anima_disAppear(pm_page, &back_options, _back_disAppear_complete_cb);
    // 3. 准备上一页
    lv_pm_history_len--;
    uint8_t prev_pid = lv_pm_history[lv_pm_history_len - 1];
    lv_pm_page_t *prev_pm_page = lv_pm_router[prev_pid]; // 将要出现的上一页
    lv_obj_t *prev_page = prev_pm_page->page;
    // 4. 让上一页执行出现动画 (也使用 back_options)
    if (prev_pm_page->willAppear) {
        prev_pm_page->willAppear(prev_page);
    }
    lv_obj_clear_flag(prev_pm_page->page, LV_OBJ_FLAG_HIDDEN);
    _pm_anima_appear(prev_pm_page, &back_options, _back_appear_complete_cb);
    return 0;
}
