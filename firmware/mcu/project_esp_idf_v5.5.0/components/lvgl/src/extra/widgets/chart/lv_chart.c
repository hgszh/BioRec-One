 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_chart_class = {.constructor_cb = lv_chart_constructor,
...
void lv_chart_set_range(lv_obj_t *obj, lv_chart_axis_t axis, lv_coord_t min, lv_coord_t max) {
    ...
    case LV_CHART_AXIS_SECONDARY_X:
        ...
        chart->xmax[1] = max;
        break;
    default: LV_LOG_WARN("Invalid axis: %d", axis); return;
    }
...
void lv_chart_set_axis_tick(lv_obj_t *obj, lv_chart_axis_t axis, lv_coord_t major_len, lv_coord_t minor_len, lv_coord_t major_cnt,
                            lv_coord_t minor_cnt, bool label_en, lv_coord_t draw_size) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
...
// https://forum.lvgl.io/t/add-space-in-chart-in-circular-mode/6818/12
void lv_chart_set_next_value(lv_obj_t *obj, lv_chart_series_t *ser, lv_coord_t value, uint8_t space_len) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
    LV_ASSERT_NULL(ser);
    ...
    for (uint8_t i = 1; i < space_len; i++) {
        ...
        else ser->y_points[ser->start_point + i] = LV_CHART_POINT_NONE;
    }
    invalidate_point(obj, ser->start_point);
    ser->start_point = (ser->start_point + 1) % chart->point_cnt;
    invalidate_point(obj, ser->start_point);
...
static void draw_series_bar(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx) {
    ...
    uint32_t ser_cnt = _lv_ll_get_len(&chart->series_ll);
    int32_t block_gap =
        ((int32_t)lv_obj_get_style_pad_column(obj, LV_PART_MAIN) * chart->zoom_x) >> 8; /*Gap between the column on ~adjacent X*/
    lv_coord_t block_w = (w - ((chart->point_cnt - 1) * block_gap)) / chart->point_cnt;
    int32_t ser_gap = ((int32_t)lv_obj_get_style_pad_column(obj, LV_PART_ITEMS) * chart->zoom_x) >>
                      8; /*Gap between the columns on the ~same X*/
...
static void draw_y_ticks(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, lv_chart_axis_t axis) {
    ...
    for (i = 0; i <= total_tick_num; i++) {
        ...
        if (p1.y + line_dsc.width / 2 >= obj->coords.y1 && p2.y - line_dsc.width / 2 <= obj->coords.y2) {
            lv_draw_line(draw_ctx, &line_dsc, &p1, &p2);
        }
...
static void draw_x_ticks(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, lv_chart_axis_t axis) {
    ...
    if (chart->type == LV_CHART_TYPE_BAR) {
        int32_t block_gap = ((int32_t)lv_obj_get_style_pad_column(obj, LV_PART_MAIN) * chart->zoom_x) >>
    ...
    for (i = 0; i <= total_tick_num; i++) { /*one extra loop - it may not exist in the list, empty label*/
        ...
        if (p1.x + line_dsc.width / 2 >= obj->coords.x1 && p2.x - line_dsc.width / 2 <= obj->coords.x2) {
            lv_draw_line(draw_ctx, &line_dsc, &p1, &p2);
        }
    ...
    if (ser->start_point != 0) {
        ...
        if (cnt >= point_cnt_old) {
            for (i = 0; i < point_cnt_old; i++) {
                new_points[i] = (*a)[(i + ser->start_point) % point_cnt_old]; /*Copy old contents to new array*/
            }
